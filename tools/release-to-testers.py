...
BUNDLE = os.environ.get("APP_BUNDLE_ID", "com.schabbauer.AMSCoffee")
...
# Apple processes for a few minutes before the build exists to be released.
build_id = None
for attempt in range(40):
    code, builds = call("GET", "/v1/builds" + q({"filter[app]": app_id, "limit": 20}))
    for b in builds.get("data", []):
        if b["attributes"].get("version") == VERSION:
            build_id = b["id"]
            state = b["attributes"].get("processingState")
            print(f"build {VERSION} is {state}")
...
if group is None:
    sys.exit(f"::error::no tester group called '{GROUP_NAME}'. "
             "Nothing will reach the phone until one exists.")

code, _ = call("POST", f"/v1/betaGroups/{group['id']}/relationships/builds",
               {"data": [{"type": "builds", "id": build_id}]})
if code not in (200, 201, 204):
    sys.exit(f"::error::could not release the build to '{GROUP_NAME}': HTTP {code}")
